...
- `pragmai update`: desde 0.7.18, actualiza el ejecutable de Windows con descarga verificada, sincronización y diagnóstico;
- `pragmai update-status`: informa si la actualización de Windows está pendiente, completada o fallida;
...
- nunca incluir prompts, respuestas, comandos, argumentos, nombres de archivos, rutas, URLs, transcripciones, identificadores de sesión, nombres arbitrarios de herramientas ni nombres personalizados de plugins o skills en la telemetría; sólo se admiten etiquetas públicas incluidas en catálogos cerrados y cualquier nombre no reconocido se agrega como `other`;
...
- Reuse a verified connection only when its identity, scope, and permissions fit the current task. Do not create new accounts, service identities, credentials, integrations, or broad permissions without explicit authorization. Request missing access, apply least privilege, limit operations to the required records or ranges, and validate the result.
...
En Windows, desde 0.7.18, `pragmai update` selecciona la versión del manifiesto firmado, intenta descargarla mediante WinGet y usa el release oficial de GitHub si WinGet no está instalado o no tiene el paquete o la versión. La autorización para ejecutar `update` incluye esa alternativa oficial; no se vuelve a pedir confirmación. Errores de integridad, certificados, políticas, permisos o cancelación detienen la actualización. Se verifica el SHA-256 publicado por GitHub antes de extraer un único ejecutable y comprobar su versión. Un proceso separado espera a que Windows libere el archivo anterior, conserva respaldo y ejecuta `repair` y `doctor`. El anuncio de inicio no implica éxito: se espera la confirmación del proceso auxiliar o se consulta `update-status`. Este estado técnico local no contiene telemetría, correo ni credenciales.

Las versiones de Windows hasta 0.7.17 requieren una actualización inicial asistida: con autorización, el asistente descarga el ZIP oficial del release indicado por `check-update`, verifica su hash y ejecuta `repair` y `doctor` con el ejecutable nuevo. No se requiere reinstalar ni volver a vincular. macOS y Linux conservan la actualización mediante paquete o artefacto oficial.
...
6. verificar ausencia de texto libre, nombres arbitrarios o personalizados de herramientas, comandos, argumentos, rutas, URLs e identificadores de sesión; las etiquetas públicas deben pertenecer a los catálogos cerrados;
