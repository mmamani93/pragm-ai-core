...
El ahorro por compactación requiere una transición `compact_boundary` observable. El conector reconoce tanto el registro directo como la variante `system`/`compact_boundary` y excluye del corte de intercambios el resumen sintético marcado `isCompactSummary`. Mientras no capture una transición, el panel muestra que la medición no tiene evidencia suficiente —no que Claude necesariamente haya compactado cero veces— y no inventa tarjetas de ahorro. El aviso administrado de una actualización disponible no constituye por sí mismo un desvío del perfil de compactación.
