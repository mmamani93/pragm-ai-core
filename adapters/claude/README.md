...
- tiempo activo observado, estimado desde intervalos del transcript con pausas limitadas a cinco minutos;
- líneas de código, cantidad y resultado estructural de ediciones, commits, PR y subagentes;
- llamadas a plugins y skills, con etiquetas sólo para productos públicos aprobados y `other` para cualquier nombre personalizado;
...
Claude Code no expone una unidad de créditos de suscripción equivalente a la publicada para Codex. Por eso `credits_used` queda sin cobertura para Claude en vez de inferirse desde tokens; el equivalente API sí se calcula con la tarifa pública vigente. El panel omite métricas de créditos en vistas exclusivas de Claude y muestra en su lugar el porcentaje de input leído desde caché. Cuando la telemetría distingue escritura de caché de una hora, se conserva ese volumen para aplicar su tarifa específica.

La detección usa el marcador nativo `compact_boundary`, directo o dentro de `system`, y admite metadatos `compactMetadata` y `compact_metadata`. El hook `Stop` atribuye cada marcador al primer intercambio con una llamada posterior observada, incluyendo compactaciones manuales entre mensajes. Los marcadores posteriores a la última llamada quedan pendientes hasta el siguiente intercambio: no crean filas adicionales ni se vuelven a contar en intercambios posteriores. Una compactación sin uso posterior todavía no se reporta. Se excluyen resúmenes `isCompactSummary` y mensajes internos `isMeta` del corte humano; si el transcript sólo conserva el resumen, se captura la continuación sin clasificar su texto ni generar recurrencia. El tamaño del contexto compactado sólo se informa si el cliente lo expone; la entrada de la primera llamada posterior se mide por separado y no se presenta como tamaño exacto del resumen. Mientras no capture una transición, el panel muestra que la medición no tiene evidencia suficiente —no que Claude necesariamente haya compactado cero veces— y no inventa tarjetas de ahorro. El aviso administrado de una actualización disponible no constituye por sí mismo un desvío del perfil de compactación.
...
En ON, el conector limita por defecto la respuesta final a 150 palabras o cinco viñetas —sin recortar exactitud, seguridad ni contenido necesario—, agrega instrucciones persistentes para un checkpoint objetivo de hasta 10.000 tokens y configura `CLAUDE_AUTOCOMPACT_PCT_OVERRIDE=64` sobre una referencia de 200.000 tokens. En OFF restaura el valor previo y retira sólo las reglas de optimización.
